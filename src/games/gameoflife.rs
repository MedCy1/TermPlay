...
impl GameOfLife {
    pub fn new() -> Self {
        ...
        game.place_pattern(
            Pattern::Blinker,
            game.grid_width / 2 - 10,
            game.grid_height / 2 - 5,
        );
        game.place_pattern(
            Pattern::Block,
            game.grid_width / 2 + 10,
...
impl Game for GameOfLife {
    ...
    fn handle_key(&mut self, key: KeyEvent) -> GameAction {
        match key.code {
            ...
            KeyCode::Char('x') => {
                // Utilise 'x' au lieu de 'n' car 'n' est déjà utilisé pour step
...
fn draw_game_of_life(frame: &mut ratatui::Frame, game: &GameOfLife) {
    ...
    let grid_start_x =
        inner_area.x + (inner_area.width as usize).saturating_sub(total_grid_width) as u16 / 2;
    let grid_start_y =
